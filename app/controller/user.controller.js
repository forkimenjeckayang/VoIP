...
exports.register = async (req, res) => {
    try {
        ...
        res.status(400).json({ status: 'false', message: 'something is wrong', error: error.message });
    }
};
                ...
                try {
                    ...
                    try {
                        ...
                        if (settings[i].app_key) {
                            await twilioHelper.removeAPIKey(settings[i].app_key)
                        }
                        if (settings[i].sid) {
                            try {
                                await twilioHelper.unlinkNumber(settings[i].sid)
                            } catch (error) { }
                        }
                    } catch (error) {
